...
module RGame
  module Engine
    ...
    #
    # A frame has three kinds of content. **World** content is drawn once per
    # viewport, inside a WorldView. A **player's own screen space** — their HUD,
    # their inventory — is drawn once, clipped to their viewport. **Global
    # screen space** — a cutscene, a results panel — is drawn once across the
    # whole window.
    #
    ...
    # is not harmless, and nothing about drawing a HUD after the world makes it
    # appear above the world. Only its z does.
    #
    # ## A z band is not a space
    #
    # Worth keeping apart, because "band" gets used for both elsewhere.
    #
    # **Space** is structural, and the tree enforces it: a node is inside a
    # WorldView or it is not, which decides what its coordinates mean and how
    # many times it is drawn. **A z band** is an ordering convention, and
    # nothing enforces it at all.
    #
    # They are not the same partition. Everything outside a WorldView is one
    # space, drawn once; `HUD` and `OVERLAY` divide *that* space by what should
    # cover what. Until a player's own screen-space node exists, `HUD` names a
    # band whose structural counterpart has not been built yet.
    #
    # ## The bands
    #
