...
RSpec.describe Engine::Components::CollisionWorld do
  ...
  def place(x, y, layer)
    ...
    collider = node.add_component(Engine::Components::CircleCollider.new(radius: 12, layer: layer))
  ...
  def tick
    scene.control(Engine::Actions.new) # resolve absolute positions
    world.update(0.0)
  end
  ...
  describe '#query_circle' do
    ...
    it 'yields nothing before the first update has built the index' do
      place(100, 100, :enemy)
      scene.control(Engine::Actions.new) # resolves positions but does not run the world
      expect(in_circle(100, 100, 50)).to be_empty
    end
  end
