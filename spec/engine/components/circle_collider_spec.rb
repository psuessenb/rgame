...
RSpec.describe Engine::Components::CircleCollider do
  ...
  def collider_at(x, y, radius)
    parent = Engine::Node2D.new
    node = parent.add_node(Engine::Node2D.new(x: x, y: y))
    collider = node.add_component(described_class.new(radius: radius))
    parent.control(Engine::Actions.new) # resolve abs_x/abs_y
    collider
  end
  ...
  describe 'tree lifecycle' do
    ...
    let!(:world) { scene.add_component(Engine::Components::CollisionWorld.new(cell_size: 64)) }
  ...
  describe '#emit_hit' do
    it 'notifies listeners connected via #on_hit with the other collider' do
      collider = Engine::Node2D.new.add_component(described_class.new(radius: 5))
