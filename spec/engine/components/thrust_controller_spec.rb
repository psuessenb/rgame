...
RSpec.describe Engine::Components::ThrustController do
  subject(:controller) do
    described_class.new(turn_speed: 4.0, accel: 100.0, max_speed: 200.0)
  end

  let(:node)     { Engine::Node2D.new }
  let(:velocity) { Engine::Components::Velocity.new }
  ...
  describe '#control' do
    it 'maps the turn axis to angular velocity' do
      controller.control(Engine::Actions.new(axes: { turn: 1.0 }))
  ...
  describe '#update' do
    it 'accelerates along the heading — angle 0 faces +x, so it moves right' do
      controller.control(Engine::Actions.new(axes: { thrust: 1.0 })) # node.angle stays 0 = +x
    ...
    it 'does not accelerate without thrust input' do
      controller.control(Engine::Actions.new(axes: { thrust: 0.0 }))
      controller.update(0.1)
      expect([velocity.vx, velocity.vy]).to eq([0.0, 0.0])
    end

    it 'clamps the resulting speed to max_speed' do
      controller.control(Engine::Actions.new(axes: { thrust: 1.0 }))
