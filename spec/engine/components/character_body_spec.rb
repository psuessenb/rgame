...
RSpec.describe Engine::Components::CharacterBody do
  ...
  let(:node)  { Engine::Node2D.new(x: 100.0, y: 100.0, width: 32, height: 32) }
  let(:world) { instance_double(Engine::Components::TileWorld, move: nil) }
  let(:body)  { described_class.new(feet_width: 16, feet_height: 16, speed: 50.0) }

  before do
    allow(node).to receive(:system).with(Engine::Components::TileWorld).and_return(world)
