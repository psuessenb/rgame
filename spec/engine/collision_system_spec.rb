...
RSpec.describe Engine::CollisionSystem do
  # Minimal actor: just what the system touches (x, y, collision_box).
  let(:actor_class) { Struct.new(:x, :y, :collision_box) }
  let(:box) { Engine::CollisionBox.new(offset_x: 8, offset_y: 16, width: 16, height: 16) }
  ...
  def system(solid:, world_width: 1000, world_height: 1000)
    described_class.new(
      tile_collision: Engine::TileCollision.new(tile_width: 16, tile_height: 16, solid: solid),
