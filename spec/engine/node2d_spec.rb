...
class SpecHealthComponent < Engine::Component; end
class SpecPhysicsComponent < Engine::Component; end
...
class SpecLifecycleComponent < Engine::Component
  attr_accessor :log
...
class SpecLifecycleNode < Engine::Node2D
  attr_accessor :log
...
class SpecRecordingNode < Engine::Node2D
  def initialize(log)
    super()
    @log = log
  end
...
RSpec.describe Engine::Node2D do
  ...
  describe '#get_component' do
    ...
    it 'matches by ancestry, so a base class finds a subclass instance' do
      component = SpecHealthComponent.new
      node.add_component(component)
      expect(node.get_component(Engine::Component)).to be(component)
    end
  end
  ...
  describe '#remove_component' do
    ...
    it 'matches by ancestry, so a base class removes a subclass instance' do
      expect(node.remove_component(Engine::Component)).to be(component)
      expect(node.components).to eq([])
    end
  ...
  describe 'deferred free' do
    ...
    describe '#sweep_freed' do
      ...
      it 'forwards the sweep into its components' do
        component = instance_double(Engine::Component, :node= => nil, on_attach: nil, sweep_freed: nil)
        node.add_component(component)
        node.sweep_freed
  ...
  describe 'the tick' do
    ...
    let(:log)       { [] }
    let(:component) { instance_double(Engine::Component) }
    let(:child)     { described_class.new }
