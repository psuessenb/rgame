...
/*
 * The break records two things per space: where the line would end (before the
 * space) and what that line measures to. The latter is the pen's position at
 * the glyph before the space, which is not the position the space itself is
 * drawn at — kerning into the space moves that pen a fraction of a pixel
 * further along. See kern's comment in rgame_text_cursor_next.
 */
void rgame_typeface_fit(const rgame_typeface *typeface, const char *text, size_t length,
    ...
    size_t last_space = 0;  /* byte offset of the last space seen, or the whole length */
    float last_space_width = 0.0f;
    float previous_pen = 0.0f;
    size_t space_seen = 0;

    int codepoint = 0;
    while (rgame_text_cursor_next(&cursor, typeface, &codepoint, NULL)) {
        if (codepoint == ' ') {
            last_space = cursor.offset - 1; /* one-byte, like every space here */
            last_space_width = previous_pen;
            space_seen = 1;
        }
        previous_pen = cursor.pen_x;

        if (cursor.pen_x > max_width) {
            break;
        }
    }

    if (cursor.pen_x <= max_width) {
        /* The whole string fits; the walk ran to the end and the pen is its
         * width. */
        *fit_length = length;
    ...
    if (space_seen) {
        *fit_length = last_space;
        *fit_width = last_space_width;
        return;
    }

    /* A single word wider than the whole line: walk to its end so the caller
     * makes progress, and report the word whole rather than cut. */
    size_t word_end = cursor.offset;
    while (rgame_text_cursor_next(&cursor, typeface, &codepoint, NULL)) {
        if (codepoint == ' ') {
            word_end = cursor.offset - 1;
            break;
        }
        word_end = cursor.offset;
    }

    *fit_length = word_end;
    *fit_width = rgame_typeface_measure(typeface, text, word_end);
}
