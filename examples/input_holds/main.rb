...
module InputHoldsExample
  ...
  class Chest < Engine::Node2D
    ...
    BODY = Util::Color.new(107, 74, 47)
    OPENED = Util::Color.new(201, 162, 39)
    SEARCHED = Util::Color.new(232, 217, 160)
    METER = Util::Color.new(79, 163, 209)

    STATE_COLOR = { shut: BODY, open: OPENED, searched: SEARCHED }.freeze

    STATE = {
      shut: Engine::Text.new('chest.shut'),
      open: Engine::Text.new('chest.open'),
      searched: Engine::Text.new('chest.searched')
    }.freeze
    ...
    def click = system!(Engine::AudioOut).play_sound(CLICK)
  end
  ...
  class Stance < Engine::Node2D
    SIZE = 56

    SWORD = Util::Color.new(176, 65, 62)
    SHIELD = Util::Color.new(62, 111, 176)

    NAME = {
      sword: Engine::Text.new('stance.sword'),
      shield: Engine::Text.new('stance.shield')
    }.freeze

    GUARD = {
      true => Engine::Text.new('status.blocking'),
      false => Engine::Text.new('status.open')
    }.freeze
