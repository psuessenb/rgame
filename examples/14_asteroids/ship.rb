...
class Ship < Engine::Node2D
  ...
  signal :on_fire, Engine::Signal.define(:x, :y, :angle)
  signal :on_destroyed
  ...
  def initialize(world_width:, world_height:)
    ...
    add_component(Engine::Components::ScreenWrap.new(width: world_width, height: world_height, margin: RADIUS))
    add_component(Engine::Components::Sprite.new(id: :ship, z: ENTITY_Z))
