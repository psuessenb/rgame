...
module JumpTopdownExample
  ...
  class Hero < Engine::Node2D
    ...
    SHADOW = Util::Color.rgba(0, 0, 0, 90)
  ...
  class Scene < Engine::Node2D
    STATE = { true => Engine::Text.new('status.airborne'),
              false => Engine::Text.new('status.grounded') }.freeze

    def initialize
      super(band: :overlay)
      @help_walk = Engine::Text.new('help.walk')
      @help_box = Engine::Text.new('help.box')
      @help_fence = Engine::Text.new('help.fence')
    end
