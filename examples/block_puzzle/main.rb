...
class Block < RGame::Engine::Node2D
  SLIDE = 0.15
  ...
  def initialize(world:, col:, row:)
    ...
    @from_y = y
    @home = false
    add_component(RGame::Engine::Components::OccupiesCell.new(col:, row:))
    @slide = add_component(RGame::Engine::Components::Tween.new(SLIDE))
  end
  ...
  def shove(step_col, step_row)
    ...
    remove_component(RGame::Engine::Components::OccupiesCell)
    add_component(RGame::Engine::Components::OccupiesCell.new(col:, row:))
    @col = col
    @row = row
    @from_x = x
...
class Hero < RGame::Engine::Node2D
  ...
  def initialize(room:, **)
    ...
    add_component(RGame::Engine::Components::BoxCollider.new(width: HERO_SIZE, height: HERO_SIZE,
                                                             layer: :hero))
    @body = add_component(RGame::Engine::Components::CharacterBody.new(speed: HERO_SPEED,
                                                                       blocked_by: [:tiles]))
...
class Hud < RGame::Engine::Node2D
  def initialize(room:)
    ...
    @solved = RGame::Engine::Text.new('hud.solved')
  end
