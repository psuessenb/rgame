...
module MusicExample
  ...
  class Scene < Engine::Node2D
    ...
    BAR_W = 520
    BAR_H = 26
    RAIL = Util::Color.new(40, 48, 66)
    FILL = Util::Color.new(120, 200, 255)
    FILL_PAUSED = Util::Color.new(90, 110, 140)
    STATE = {
      stopped: Engine::Text.new('status.stopped'),
      rising: Engine::Text.new('status.rising'),
      playing: Engine::Text.new('status.playing'),
      falling: Engine::Text.new('status.falling'),
      paused: Engine::Text.new('status.paused')
    }.freeze
    ...
    def initialize
      ...
      @music = 10 # tenths
      @effects = 10
      @playhead = Engine::Tween.new(LOOP_SECONDS, loop: true)
      @help = Engine::Text.new('help.keys')
      @help_volume = Engine::Text.new('help.volume')
      @volumes = Engine::Text.new('status.volumes', :music, :effects)
    end
    ...
    def out = system!(Engine::AudioOut)
