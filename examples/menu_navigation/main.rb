...
module MenuNavigationExample
  ...
  class Settings
    ...
    #
    # **These live in a class, and a proc is why.** A block captures the local
    # variables of the scope it is written in, and a constant holding that block
    # keeps them for the life of the process. Were the game one of them, its
    # window would never be released, which shows up not as a leak but as a crash
    # on the way out. A class body has no locals to capture, and `start` below
    # keeps `game` local to itself for the same reason.
    ROWS = {
  ...
  class TitleScene < Engine::Node2D
    MENU_X = 56
    MENU_Y = 104
    ITEM_WIDTH = 240
    ITEM_HEIGHT = 34
    ...
    def initialize(**)
      super
      @heading = Engine::Text.new('title.heading')
      @help = Engine::Text.new('title.help')
    end
  ...
  class SettingsScene < Engine::Node2D
    PANEL_X = 28
    PANEL_Y = 28
    ...
    def initialize(settings:, **)
      super(**)
      @settings = settings
      @heading = Engine::Text.new('settings.heading')
    end
  ...
  class PlayScene < Engine::Node2D
    ...
    def initialize(**)
      super
      @phase = 0.0
      @help = Engine::Text.new('play.help')
    end

    def _control(actions)
      system!(Engine::AudioOut).play_sound(BLIP) if actions.pressed?(:ui_confirm)
  ...
  def self.start
    ...
    settings = Settings.new(
      Util::SaveFile.new('settings.json', game: 'rgame-examples',
