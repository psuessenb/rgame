...
class Piece
  ...
  def initialize(key, slot:, centre_x:, centre_y:)
    @name = RGame::Engine::Text.new(key, scope: 'pieces')
    @slot = slot
    @centre_x = centre_x
    @centre_y = centre_y
  end
...
class Figure < RGame::Engine::Node2D
  SHEET = 'hero.json'
  SCALE = 5
  WIDTH = 112
  HEIGHT = 160
...
class PieceButton < RGame::Engine::UI::Button
  STYLE = RGame::Engine::UI::ShapeStyle.new
  SCALE = 3
...
class SlotButton < RGame::Engine::UI::PanelButton
  NAME_X = 16
  WORN_X = 80
  ...
  def initialize(slot:, outfit:, **)
    ...
    @slot = slot
    @outfit = outfit
    @empty = RGame::Engine::Text.new('outfit.empty')
  end
...
# worn.
class PiecePanel < RGame::Engine::Node2D
  WIDTH = 236
  HEIGHT = 72
  ...
  def initialize(menu:, outfit:, **)
    ...
    @menu = menu
    @outfit = outfit
    @worn = RGame::Engine::Text.new('outfit.worn')
    @carried = RGame::Engine::Text.new('outfit.carried')
  end
...
class Wardrobe < RGame::Engine::Node2D
  UI = RGame::Engine::UI
