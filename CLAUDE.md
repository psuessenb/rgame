...
`lib/rgame/rubocop/cop/game/` holds seven project-specific cops (plus the shared
...
project generated by `rgame new` runs the five game-facing ones plus
...
given, so neither sees what a whole game allocates. `TileMap#frame_tile`
allocated for every animated tile in view, every frame, with every spec green:
no spec's map showed a frame but the last. `rake drive:allocations` is the
check that sees the whole game. It drives every example and test project with
its script and fails one that allocates over its budget once warm (see
"Testing" below).
...
The first four are expected to pass for every change, and CI runs all four.
