...
class Hero < RGame::Engine::Node2D
  ...
  def initialize(camera:, **)
    ...
    add_component(RGame::Engine::Components::FeetCollider.new(width: FEET_WIDTH, height: FEET_HEIGHT,
                                                              layer: :hero))
    add_component(RGame::Engine::Components::CharacterBody.new(speed: SPEED, blocked_by: %i[tiles crate npc],
                                                               pushes: [:crate]))
    ...
    add_component(RGame::Engine::Components::Footing.new).on_fell { fell }
