...
module Asteroids
  ...
  class PlayScene < Engine::Node2D
    ...
    def _enter_tree
      ...
      system!(Engine::AudioOut).play_music(:heartbeat)
    end

    def _exit_tree = system!(Engine::AudioOut).stop_music
    ...
    def fire_bullet(x, y, angle)
      ...
      add_node(bullet)
      system!(Engine::AudioOut).play_sound(:shoot)
    end
    ...
    def lose
      system!(Engine::AudioOut).play_sound(:hurt)
