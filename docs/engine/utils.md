...
## `Engine::Matrix` / `Engine::Tensor` — flat fixed-size grids

`Engine::Matrix` (`engine/matrix`) and `Engine::Tensor` (`engine/tensor`) are fixed-size
grids addressed as `[x, y]` / `[x, y, z]` but backed by a **single flat (row-major) array**, not
an array-of-arrays. One contiguous allocation is cheaper than nested arrays and is the shape a
later C-level buffer would take — so reaching for these now makes that move a drop-in.

```ruby
grid = Engine::Matrix.new(width, height, initial: 0)
grid[col, row] = gid
grid[col, row]                      # row-major: index = y * width + x

cube = Engine::Tensor.new(width, height, depth, initial: 0)
cube[col, row, layer] = gid         # x-fastest, then y, then z
```

They do no bounds checking on the hot path (callers stay in range). `TileMap` stores its stacked
tile layers in one `Tensor(width, height, layer_count)`.

## `Engine::CachedLabel` — a display string rebuilt only on change

`Engine::CachedLabel` (`engine/cached_label`) holds a label string and rebuilds it only when its
...
```ruby
@score_label = Engine::CachedLabel.new { |score| "Score: #{score}" }  # built once
...
help — draw the digits individually from cached glyph strings instead, as `Engine::DebugOverlay` does.

## `Engine::Pool` — reuse, don't allocate

`Engine::Pool` (`engine/pool`) recycles many short-lived, homogeneous objects —
...
```ruby
pool = Engine::Pool.new { Bullet.new }   # factory builds a blank object
...
(typically via a `reset` from an [`Engine::Resettable`](#engineresettable--mutable-only-where-a-pool-needs-it)
...
## `Engine::Path` — a walkable polyline

`Engine::Path` (`engine/path`) is an ordered polyline of waypoints an entity walks along —
...
```ruby
path = Engine::Path.new([[0, 0], [100, 0], [100, 100]]) # ≥ 2 waypoints, in walk order
...
## `Engine::Timer` — paced periodic events

`Engine::Timer` (`engine/timer`) is a repeating interval timer for periodic events that
...
```ruby
@spawn_timer = Engine::Timer.new(0.8)   # built once, off the hot path
...
## `Engine::Camera` — follow a point, clamp to the world

`Engine::Camera` (`engine/camera`) is the pure follow-and-clamp maths for a scrolling
...
## `Engine::CollisionBox` — an actor's feet box

`Engine::CollisionBox` (`engine/collision_box`) is a character's collision rectangle,
...
## `Engine::I18n` — localization

`Engine::I18n` (`engine/i18n`) is minimal localization: per-locale translation tables
...
```ruby
Engine::I18n.load_file(:en, "locales/en.yml")
Engine::I18n.load(:de, menu: { title: "Hauptmenü" })   # nested Hashes allowed
Engine::I18n.default = :en        # fallback when the current locale lacks a key
Engine::I18n.locale = :de

Engine::I18n.t("menu.title")               # dotted key, resolved in :de then :en
Engine::I18n.t(:greeting, name: "Ada")     # => "Hello, Ada" from %{name}
Engine::I18n.t(:apples, count: 3)          # pluralized: { one:, other:, zero? }
```
...
## `Engine::AudioBus` — decoupled audio facts

`Engine::AudioBus` (`engine/audio_bus`) is a global, always-present audio bus: gameplay
emits audio *facts* (`play this sound`, `play this music`) here, decoupled from any Gosu
playback, and a platform-side `AudioDirector` subscribes and turns them into actual
sound. A module rather than an instance so any node can reach it without wiring.

```ruby
Engine::AudioBus.play_sound(:boom)
Engine::AudioBus.play_music(:theme)
Engine::AudioBus.stop_music
```

The emit shims (`play_sound`, `play_music`, `stop_music`) are the gameplay-facing API;
underneath each is an [`Engine::Signal`](signals.md) (`on_play_sound`, `on_play_music`,
`on_stop_music`) that is the actual subscription seam the director listens on. Because
the engine only emits facts and never touches Gosu, the bus stays in the engine core and
playback stays in the platform layer.

## `Engine::Resettable` — mutable only where a pool needs it

`Engine::Resettable` (`engine/resettable`) builds value-object classes for pooling. Like
...
Vel = Engine::Resettable.define(:dx, :dy, keyword_init: true)
Vel.new(dx: 1, dy: 0).reset(dx: 2, dy: 0)
```

That single in-place `reset` is the only mutability a [`Pool`](#enginepool--reuse-dont-allocate)
