...
- **Verify** (the full sweep, and the one that matters most in this phase):
  ...
  ```
  grep -ril gosu lib ext spec spec_core examples rubocop Gemfile Gemfile.lock \
            .rubocop.yml rgame.gemspec
  ```

  No *dependency* may survive it. Mentions may, in exactly two shapes: comments
  that teach a difference a reader coming from Gosu will hit (the mirroring
  convention, in `primitives.h` and the sprite-sheet spec), and pointers into
  this folder, which 6.10 replaces with the content they point at. Everything
  else goes.

**Landed, and phase 6 with it.** `lib/platform/` is gone, `gosu` is out of the
`Gemfile` and the lockfile, and nothing in the project depends on it in any
form.

**`Game/PreferGosuModuleMethod` retired** — the cop, its `require` line, its
`Exclude` block and its rows in `.rubocop.yml` and CLAUDE.md. Five cops now. The
principle it enforced is structural rather than linted: the C bindings have
fixed arity and there is no compat shim to prefer against.

**The packaging exclusion changed hands rather than going away.** `lib/platform/`
came out of the gemspec and the packaging spec; `lib/engine/`, `lib/engine.rb`
and `lib/boot.rb` went in, and the reason is different. The platform layer was
excluded because it *could not work* in the gem — gosu was not a dependency. The
engine layer works fine and depends on nothing; it is held back because it is
still a bare top-level `Engine::`, and a gem has no business putting that
constant into someone's process. It ships when it moves. The packaging spec
states the pattern independently of the gemspec, as before, so the two have to
agree rather than share a mistake.

**Two documents amended where they had gone stale**, both outstanding since
phase 1–2. `docs/c_engine_feature_specs.md` §1 said the fixed-timestep
accumulator "stays on the engine side of the boundary" — it is `frame_loop.c`,
and the line predates that file existing; and §1 and §5 listed mouse state as
required, which was dropped deliberately. Both now say so and link the decision.

**A sweep of the word itself** turned up eight stale references in code we own,
distinct from the ones that teach something:

- `spec/spec_helper.rb` claimed to require `lib/engine` (it requires
  `lib/rgame`) and framed the guarantee as "no Gosu loaded" rather than what it
  actually is — no SDL, and `RGame::Core` undefined.
- `transform.h` documented the clockwise-rotation fact by describing an
  experiment nobody can now repeat (`Gosu.rotate(90, …)` off-screen). Reworded
  to the fact plus the spec that still checks it.
- `input.h` explained per-frame sampling by what the old layer did. Replaced
  with what per-frame sampling does *not* fix — edge detection — pointing at
  6.8's finding.
- Four more attributions in `core.h`, `transform.c`, `draw_queue.h` and
  `x_keys.rb` that named Gosu for facts that are now simply ours.

What is left is two comments that teach the mirroring convention a Gosu reader
would otherwise trip on, plus four pointers into this folder — which is 6.10's
job, and the reason 6.10 exists.
